...
void exibirMatriz(int nVertice, int mAdjacencia[][TAM])
{
	int i, j;
	//printf(" ");
	for(i = 0; i < nVertice; i++)
...
void exibirVertices(Vertice * verticeVisitado)
{
	if(verticeVisitado == NULL)
		return;
	//printf("ID: %d Peso: %d  ant %d	",verticeVisitado->id, verticeVisitado->peso, verticeVisitado->ant);
...
int main()
{
	...
	//exibirMatriz(nVertice, mAdjacencia);
